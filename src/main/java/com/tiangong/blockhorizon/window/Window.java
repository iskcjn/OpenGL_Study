package com.tiangong.blockhorizon.window;
...
public class Window {
    ...
    // 窗口标题
    public static String TITLE = "Block Horizon";
    // 窗口句柄
    public static long WINDOW_HANDLE;

    // 是否允许窗口大小变化
    public static boolean _canResize = false;
    // 是否开启线框模式
    ...
    public void init(){
        ...
        if(!glfwInit()){
            throw new IllegalStateException("初始化 GLFW 失败");
        }
        // 打印GLFW版本
        ...
        glfwSetKeyCallback(WINDOW_HANDLE, (window, key, scancode, action, mods) -> {
            ...
            if(key == GLFW_KEY_F2 && action == GLFW_PRESS){
                // 传送
                Game._camera.setPosition(new Vector3f(0.0f, -5.0f, 0.0f));
            }
            ...
            // 鼠标移动 （用于视角移动）
            glfwSetCursorPosCallback(WINDOW_HANDLE, (win, xpos, ypos) -> {
                //System.out.println(xpos + " " + ypos);
                Game._camera.processMouseMovement(xpos, ypos);
            });
            glfwSetScrollCallback(WINDOW_HANDLE, (win, xoffset, yoffset) -> {
